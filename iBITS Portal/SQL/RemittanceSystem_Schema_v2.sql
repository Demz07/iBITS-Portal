-- ============================================================
-- SQL SCRIPT: Remittance System Schema (FIXED v2)
...
-- ============================================================
-- FIX: Removed indexes on FeeStatus/FinesStatus columns
--      which were causing "invalid column type" errors
-- ============================================================
...
PRINT '============================================================';
PRINT 'REMITTANCE SYSTEM SCHEMA v2 (FIXED)';
PRINT '============================================================';
PRINT '';
...
-- Add FK to Fees if column exists
IF EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('dbo.Fees') AND name = 'FeeId')
   AND NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'FK_RemittanceItems_Fees')
BEGIN
    ALTER TABLE [dbo].[RemittanceItems]
    ADD CONSTRAINT [FK_RemittanceItems_Fees] FOREIGN KEY ([FeeId]) 
        REFERENCES [dbo].[Fees]([FeeId]) ON DELETE NO ACTION;
    PRINT 'Added FK: RemittanceItems.FeeId -> Fees';
END
GO

-- Add FK to Fines if column exists
IF EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('dbo.Fines') AND name = 'FineId')
   AND NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'FK_RemittanceItems_Fines')
BEGIN
    ALTER TABLE [dbo].[RemittanceItems]
    ADD CONSTRAINT [FK_RemittanceItems_Fines] FOREIGN KEY ([FineId]) 
        REFERENCES [dbo].[Fines]([FineId]) ON DELETE NO ACTION;
    PRINT 'Added FK: RemittanceItems.FineId -> Fines';
END
GO

-- Add FK to Student if column exists
IF EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('dbo.Student') AND name = 'StudentNum')
   AND NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'FK_RemittanceItems_Students')
BEGIN
    ALTER TABLE [dbo].[RemittanceItems]
    ADD CONSTRAINT [FK_RemittanceItems_Students] FOREIGN KEY ([StudentNum]) 
        REFERENCES [dbo].[Student]([StudentNum]) ON DELETE NO ACTION;
    PRINT 'Added FK: RemittanceItems.StudentNum -> Student';
END
GO
...
-- NOTE: Only on columns with compatible types
-- ============================================================
...
-- Index on Fees.RemittanceStatus ONLY (not including FeeStatus)
...
BEGIN
    ...
    ON [dbo].[Fees]([RemittanceStatus]);
    
    PRINT 'Created index: IX_Fees_RemittanceStatus';
END
GO

-- Index on Fines.RemittanceStatus ONLY (not including FinesStatus)
...
SELECT 'Indexes', name 
FROM sys.indexes 
WHERE name IN ('IX_Remittances_Status', 'IX_Remittances_Section', 'IX_RemittanceItems_RemittanceId', 'IX_Fees_RemittanceStatus', 'IX_Fines_RemittanceStatus')
UNION ALL
